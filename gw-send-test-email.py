...
script_name     = 'gw-send-test-email.py'
...
parser.add_argument("-t", "--to", help="run from canned test file", action="append")
parser.add_argument("-s", "--subj", help="run from canned test file", default="[P2IoTGateway Test] test email")
...
test_email_toAddr = parse_args.to
test_email_subj = parse_args.subj
...
if test_email_toAddr == None:
    print_line("* ERROR Missig required --to option - to whom are we sending?!", error=True)
    os._exit(1)
...
emailTo = test_email_toAddr
emailFrom = "pi@" + runtimeConfig.getValueForConfigVar(runtimeConfig.keyRPiFqdn)
emailSubj = test_email_subj
emailTextLines = [
    "Hi Person",
    "   This is your Raspberry Pi sending you a test email via your configured sender!",
    "",
    "Regards",
    "Your RPi"
]
...
# send our test email
createAndSendEmail(emailTo, emailFrom, emailSubj, emailTextLines)
